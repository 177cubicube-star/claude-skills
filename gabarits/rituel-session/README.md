...
| Procédure | État |
|---|---|
| Verdict de continuité (session-prep, étape 2) | **Rodé sur dépôt jetable, 2026-09-23** : 5 cas (aucun recap, RECAP_LOCAL, OK après clôture, GAP, sans HEAD) rendent le verdict attendu, 0 verrou restant. Ce rodage a **trouvé un défaut** : sans exclure les fichiers de clôture, chaque clôture propre rendait GAP ; corrigé et re-mesuré. **Validé sur un vrai dépôt, Dr-bobo, 2026-09-23** : `INDÉTERMINÉ — aucun recap` avant le premier recap, puis **OK** après la clôture poussée (`0 commit hors sessions/ après 1495dbe`, briefing @ `0282b6a`) — le faux GAP du rodage n'est pas revenu. · **App-Handyman, 2026-09-23 — hors calibration** : `INDÉTERMINÉ — recap sans HEAD` a le format exact de sortie de `tools/recap_gap.py`, script déclaré dans la FORME de `session-prep-ah` que le fond invoque à la place de la procédure en ligne ; l'appel n'est pas consigné dans le recap (`d2cad31`). Ce verdict ne calibre pas la procédure en ligne. · **Pilote automatique, 2026-09-23** : aucun script déclaré, donc procédure en ligne — `session-prep-pa` en lecture seule avant tout recap, aucun fichier modifié, mais verdict **non cité littéralement** dans le recap (`2467989`) : non compté. Temps 3 (push puis `OK`) non exercé dans les deux instances. |
...
| Mesure de la date (v1.0.1) | **Rodée en conteneur, 2026-09-23** : fuseau connu accepté ; fuseau inconnu → repli UTC **attrapé** (le défaut réel, reproduit : `exit 0`, `+0000`) ; projet réellement en UTC **non rejeté** ; aucun instrument valide → « NON MESURÉE ». **Validée sous Windows, dans le skill, 2026-09-23 (Dr-bobo)** : `TZ` déclaré invalide (`+0000`), PowerShell retenu — `recap-drbobo` à `14:34 -04:00`, `session-prep-drbobo` à `14:48 -04:00`. · **App-Handyman, 2026-09-23** : même constat, consigné dans la FORME de `session-prep-ah` (Git Bash invalide, repli `+0000` silencieux ; PowerShell retenu) ; `recap-ah` daté `20:51 -04:00`. · **Pilote automatique, 2026-09-23** : `recap-pa` daté `21:59 -04:00`, décalage correct, instrument consigné nulle part (ni FORME ni recap). |
| Axes de sync, shortlist anti-sur-claim, gate code/docs | **Non calibrés** — gardes par vigilance. Première instance (Dr-bobo, projet sans code) : **non exercés** — ni code, ni branche de travail, ni claim à auditer. · **App-Handyman** : axes de sync et shortlist y sont remplacés par des scripts déclarés (`tools/git_sync_axes.py`, `tools/claim_audit.py`), donc **non calibrables en ligne dans ce projet, même au temps 4**. Seul le gate code/docs, sans script, y calibre le gabarit : **cas négatif exercé le 2026-09-23** (session docs dans un projet à code, `git diff --stat fc2ef19..origin/main` sur les motifs déclarés : rien — `recap-20260923-01.md` § 4.2) ; **cas positif (temps 4) non exercé**. · **Pilote automatique** : aucun script déclaré, mais aucun fichier de code déclaré non plus (recap `2467989` § 4.5) — rien de consigné comme exercé. |
