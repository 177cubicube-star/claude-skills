...
| Divergences héritées de l'audit Cowork — **moitié levée le 2026-07-27** : `tdd-enforcer` est **tranché** (voir gestes courts) ; `skill-creator` **requalifié le 2026-07-31, ce n'est plus une divergence de skill** (voir ci-dessous) | close pour ce chantier — plus aucun sujet ne subsiste ici |
| **`skill-creator` — rangement des ateliers, pas hygiène de skills.** Requalifié le 2026-07-31 sur mesure : le dossier `.claude/skills/skill-creator/` de `Suspension-intelligente` pèse **1,8 Mo, 315 fichiers suivis par git**, dont le `SKILL.md` d'Anthropic ne représente que **33 Ko — moins de 2 %**. Le reste est ton travail : sept ateliers (`portal-guard` 75 fichiers · `recap` 71 · `scraping-guard` 55 · `architecture-guard` 56 · `tdd-enforcer` 41), plus `scripts`, `eval-viewer`, `agents`, `references`, `assets`. **Le remède du 2026-07-24 — « supprimer la copie repo, garder celle d'Anthropic », annoncé sans risque (`archives/carte-skills-20260724.md` l.104) — détruirait 298 fichiers si on l'appliquait au dossier.** Il n'était sans risque que sous une hypothèse jamais vérifiée : que le dossier ne contienne que le skill. Deux gestes se cachaient sous un mot — retirer le seul `SKILL.md` (33 Ko, le skill cesse de se déclencher depuis ce projet, ce qu'il ne fait probablement jamais puisque la version du compte est servie partout), ou retirer le dossier entier (à écarter). **Ce qui reste non vérifié :** que la copie du dépôt soit réellement périmée. Son `SKILL.md` n'a qu'un commit (`117012c`, 2026-05-29, simple renommage `claude/` → `.claude/`) et **ne porte aucun champ `version:`** — la version d'Anthropic non plus n'est pas lisible depuis Claude Code. Sans `version:` des deux côtés, seul un marqueur de contenu comparerait quoi que ce soit. Le « deux mois de retard » est donc **plausible, jamais mesuré** | décision de rangement de Mathieu : ces ateliers servent-ils encore, ou vont-ils aux archives ? Rien ne se supprime avant. Se rouvre aussi si un besoin réel de `skill-creator` apparaît dans ce projet — il faudra alors mesurer l'écart par marqueur de contenu, pas par `version:` |
