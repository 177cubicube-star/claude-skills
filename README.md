...
6. **Déployer vers chaque cible** où le skill doit tourner :
   ...
   - Disque (Claude Code local) : copier `.claude/skills/<nom>/` → `~/.claude/SKILLS/<nom>/` (remplacer). **Statut au 2026-07-24 :** V0 et V0-ter (épreuve du lanceur) sont VERTES — une session ouverte par `claude-ah` lit la maison à la source. Elle reste nécessaire pour toute session ouverte par `claude` nu, qui ne voit pas la maison.
     **Correction du 2026-07-26 (V3-bis, Claude Code 2.1.220, Windows) :** cette ligne affirmait aussi que « cette copie ne la sert pas ». **C'est faux pour tout nom doublé.** Mesuré : le skill personnel l'emporte sur son homonyme atteint par `--add-dir` — et aussi sur celui du `.claude/skills/` du projet courant. Les 10 skills de la maison ayant tous un homonyme personnel, **ils sont tous servis par la copie, jamais par la source** : un `git pull` sur la maison ne change rien pour eux, et **cette étape 6 n'est donc pas optionnelle**. Ce que V0/V0-ter ont prouvé reste vrai et n'est pas rétracté : la maison EST lue à la source — mais uniquement pour les skills sans homonyme personnel. Retirer la copie reste une décision à prendre : elle est désormais instruite avec le fait mesuré et ses deux options dans `ISSUES-LOG.md`, **ISSUE-001** — en attente du décideur, et non tranchée par la mesure.
